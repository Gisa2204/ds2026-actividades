import { LibroOL } from "./clases";

const librosBuscadorForm = document.querySelector("#librosBuscador-form") as HTMLFormElement;
const buscarInput = document.querySelector("#buscar-input") as HTMLInputElement;
const resultadosUL = document.querySelector("#resultados-ul") as HTMLUListElement;
...
librosBuscadorForm.addEventListener
("submit", async (librosBuscadorEvent) => //Hay que marcarla como async porque va a tener que esperar con la llamada de obtenerLibros(q)
    {
        ...
        let librosBuscados: LibroOL[] = await obtenerLibros(q);
        ...
        librosBuscados = librosBuscados.slice(0, 10); //Dejar solamente los primeros 10 libros
